# f:\12_prj_raspi5\benchmark\optimize_deepseek_r1_7b.py
"""
Benchmark & Optimization Pipeline for DeepSeek-R1-7B on Raspberry Pi 5
1. Verifies CPU Governor is at 'performance' (2.40 GHz).
2. Builds optimized model variant 'deepseek-r1:7b-opt' with customized Modelfile (num_thread=4, num_ctx=4096, R1-tuned sampling params).
3. Executes real comparative benchmarks between baseline deepseek-r1:7b and deepseek-r1:7b-opt on algorithmic reasoning prompts.
...
PROMPTS = [
    {
        ...
        "prompt": "Solve this riddle step by step: You have a 3-liter jug and a 5-liter jug, and an unlimited supply of water. How can you measure exactly 4 liters of water using only these two jugs?"
    },
    {
        "category": "Edge Logic & Self-Correction",
        "prompt": "Analyze whether binary search can be applied to find an element in a rotated sorted array without duplicates in O(log n) time. Write down the case analysis and edge cases."
    }
]
...
def create_optimized_modelfile(ssh: paramiko.SSHClient):
    print("[*] Creating optimized Modelfile for deepseek-r1:7b via Ollama CLI...")
    modelfile_content = (
        "FROM deepseek-r1:7b\n"
        ...
        "SYSTEM \"\"\"You are an expert reasoning engine. Think carefully step by step, self-correct if needed, and provide the concise final answer.\"\"\"\n"
    )
    sftp = ssh.open_sftp()
    with sftp.open("/home/pi/Modelfile.r1_7b", "w") as f:
        f.write(modelfile_content)
    sftp.close()
    
    stdin, stdout, stderr = ssh.exec_command("ollama create deepseek-r1:7b-opt -f /home/pi/Modelfile.r1_7b")
...
def main():
    print("=================================================================")
    print(" Pi 5 DeepSeek-R1 7B Optimization & Verification Benchmark Suite")
    print("=================================================================")
    ...
    try:
        ...
        for prompt_data in PROMPTS:
            res_base = run_benchmark("deepseek-r1:7b", prompt_data, ssh)
            results.append(res_base)
            
            time.sleep(3)
            
            res_opt = run_benchmark("deepseek-r1:7b-opt", prompt_data, ssh)
            results.append(res_opt)
            
            time.sleep(3)
            
        output_file = os.path.join(os.path.dirname(__file__), "deepseek_r1_7b_opt_benchmark_results.json")
