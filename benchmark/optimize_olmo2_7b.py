# f:\12_prj_raspi5\benchmark\optimize_olmo2_7b.py
"""
Benchmark & Optimization Pipeline for OLMo-2-7B on Raspberry Pi 5
1. Verifies CPU Governor is locked at 'performance' (2.40 GHz).
2. Builds optimized model variant 'olmo2:7b-opt' with customized Modelfile (num_thread=4, num_ctx=4096, scientific rigor params).
3. Executes real comparative benchmarks between baseline olmo2:7b and olmo2:7b-opt on reproducibility and transformer numerical stability tasks.
...
PROMPTS = [
    {
        "category": "Open Science & LLM Reproducibility",
        "prompt": "Analyze why releasing intermediate checkpoints and training logs (as in Ai2 OLMo 2) is critical for empirical deep learning research. Discuss data contamination detection and weight degradation analysis."
    },
    {
        "category": "Numerical Stability & Transformer Architecture",
        "prompt": "Explain the role of QK-Norm in transformer attention layers. Why does applying LayerNorm to Query and Key projections prevent attention entropy collapse and loss spikes during FP16 training?"
    }
]
...
def create_optimized_modelfile(ssh: paramiko.SSHClient):
    print("[*] Creating optimized Modelfile for olmo2:7b via Ollama CLI...")
    modelfile_content = (
        "FROM olmo2:7b\n"
        ...
        "SYSTEM \"\"\"You are an open science machine learning researcher and systems architect. Provide rigorous, transparent, and evidence-based analysis with concise technical formulations.\"\"\"\n"
    )
    sftp = ssh.open_sftp()
    with sftp.open("/home/pi/Modelfile.olmo2_7b", "w") as f:
        f.write(modelfile_content)
    sftp.close()
    
    stdin, stdout, stderr = ssh.exec_command("ollama create olmo2:7b-opt -f /home/pi/Modelfile.olmo2_7b")
...
def main():
    print("=================================================================")
    print(" Pi 5 OLMo-2 7B Optimization & Verification Benchmark Suite")
    print("=================================================================")
    ...
    try:
        ...
        for prompt_data in PROMPTS:
            res_base = run_benchmark("olmo2:7b", prompt_data, ssh)
            results.append(res_base)
            
            time.sleep(3)
            
            res_opt = run_benchmark("olmo2:7b-opt", prompt_data, ssh)
            results.append(res_opt)
            
            time.sleep(3)
            
        output_file = os.path.join(os.path.dirname(__file__), "olmo2_7b_opt_benchmark_results.json")
