# f:\12_prj_raspi5\benchmark\optimize_gemma4_e2b.py
"""
Benchmark & Optimization Pipeline for Gemma 4 E2B on Raspberry Pi 5
1. Verifies CPU Governor is locked at 'performance' (2.40 GHz).
2. Builds optimized model variant 'gemma4:e2b-opt' with customized Modelfile (num_thread=4, num_ctx=4096, lightweight edge multimodal params).
3. Executes real comparative benchmarks between baseline gemma4:e2b and gemma4:e2b-opt on embedded robotics and depth-pruning efficiency tasks.
...
PROMPTS = [
    {
        "category": "Embedded Robotics & Low-Latency Vision Pipeline",
        "prompt": "Outline a real-time embedded control loop on Raspberry Pi 5 using Gemma 4 E2B for robotic visual navigation. Specify the sensor frame rate, token latency target (<100ms), and lightweight decision serialization."
    },
    {
        "category": "Extreme Edge Pruning & Cross-Layer Distillation",
        "prompt": "Analyze the computational benefits of depth-pruning and knowledge distillation in Gemma 4 E2B (~2.2B text backbone). Why does this architecture allow high-throughput streaming on 4-core ARM Cortex-A76?"
    }
]
...
def create_optimized_modelfile(ssh: paramiko.SSHClient):
    print("[*] Creating optimized Modelfile for gemma4:e2b via Ollama CLI...")
    modelfile_content = (
        "FROM gemma4:e2b\n"
        ...
        "SYSTEM \"\"\"You are an ultra-low-latency edge AI engineer specializing in embedded robotics, computer vision, and high-frequency IoT streaming.\"\"\"\n"
    )
    sftp = ssh.open_sftp()
    with sftp.open("/home/pi/Modelfile.gemma4_e2b", "w") as f:
        f.write(modelfile_content)
    sftp.close()
    
    stdin, stdout, stderr = ssh.exec_command("ollama create gemma4:e2b-opt -f /home/pi/Modelfile.gemma4_e2b")
...
def main():
    print("=================================================================")
    print(" Pi 5 Gemma 4 E2B Optimization & Verification Benchmark Suite")
    print("=================================================================")
    ...
    try:
        ...
        for prompt_data in PROMPTS:
            res_base = run_benchmark("gemma4:e2b", prompt_data, ssh)
            results.append(res_base)
            
            time.sleep(3)
            
            res_opt = run_benchmark("gemma4:e2b-opt", prompt_data, ssh)
            results.append(res_opt)
            
            time.sleep(3)
            
        output_file = os.path.join(os.path.dirname(__file__), "gemma4_e2b_opt_benchmark_results.json")
