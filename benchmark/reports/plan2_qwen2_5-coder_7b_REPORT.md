# 📊 Plan 2: Native llama.cpp 基準測試報告 — `qwen2.5-coder:7b`
...
> **測試時間 (Timestamp):** `2026-09-08T11:44:14.615705`
...
| **平均解碼生成速度 (Avg Generation TPS)** | **2.76 tokens/s** | 4 核心向量並行解碼速度 |
| **平均提示詞處理速度 (Avg Prompt Prefill)** | **11.5 tokens/s** | ARM NEON / DotProd 加速 Prefill |
| **實體記憶體佔用 (Peak RAM RSS)** | **0.46 GiB** | 含 Context KV Cache 與權重 |
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Standard Interaction (P512 / G128)** | 512 tokens | 128 tokens | **11.59 ± 0.03** t/s | **2.77 ± 0.0** t/s | 57.6 °C |
| **Long Context & Code Generation (P1024 / G256)** | 1024 tokens | 256 tokens | **11.41 ± 0.0** t/s | **2.76 ± 0.0** t/s | 58.2 °C |
...
2. **記憶體頻寬利用率 (Memory Bandwidth Efficiency):**
   - 模型權重為 4.36 GB，實測 Token 生成速度為 **2.76 tokens/s**。
   - 實測有效讀取頻寬為：
     $$\text{Effective Bandwidth} = 4.36 \text{ GB} \times 2.76 \text{ t/s} \approx 12.03 \text{ GB/s}$$
