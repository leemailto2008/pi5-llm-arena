# 📊 Plan 2: Native llama.cpp 基準測試報告 — gemma4:e4b
...
> **測試時間 (Timestamp):** 2026-09-09T09:57:30.274311
...
| :--- | :--- | :--- |
| **模型參數規模 (Model Parameters)** | **4.3B** | Google Gemma4 多模態架構 |
| **模型量化等級 (Quantization)** | **Q4_K_M** | 4-bit K-Quants Medium |
| **模型檔案大小 (Weight Size)** | **8.95 GB** | 複合層多模態 Blobs |
| **平均解碼生成速度 (Avg Generation TPS)** | **3.68 tokens/s** | 4 核心並行解碼速度 |
| **平均提示詞處理速度 (Avg Prompt Prefill)** | **22.50 tokens/s** | ARM NEON / DotProd 加速 Prefill |
| **實體記憶體佔用 (Peak RAM RSS)** | **5.80 GiB** | 語言模型與核心投影記憶體 |
| **SoC 核心峰值溫度 (Peak Temperature)** | **62.1 °C** | Active Cooler 散熱溫控狀態 |
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Standard Interaction (P512 / G128)** | 512 tokens | 128 tokens | **22.50** t/s | **3.68** t/s | 62.1 °C |

---

## 3. 架構師效能與硬體分析 (Architectural Analysis)

1. **多模態運算效能 (High-Precision Multimodal):**
   - 在 4.3B 規模下，Token 生成速度達到 3.68 t/s，Prefill 速度達到 22.50 t/s，滿足邊緣視覺與複雜文本分析應用。
