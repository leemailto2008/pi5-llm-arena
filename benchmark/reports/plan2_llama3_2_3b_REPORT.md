# 📊 Plan 2: Native llama.cpp 基準測試報告 — `llama3.2:3b`
...
> **測試時間 (Timestamp):** `2026-09-08T11:04:38.192066`
...
| :--- | :--- | :--- |
| **模型參數規模 (Model Parameters)** | **3.21B** | 官方發行架構規模 |
| **模型量化等級 (Quantization)** | **Q4_K_M** | 4-bit K-Quants Medium |
| **模型檔案大小 (Weight Size)** | **1.88 GB** | POSIX `mmap` 零副本載入 |
| **平均解碼生成速度 (Avg Generation TPS)** | **5.71 tokens/s** | 4 核心向量並行解碼速度 |
| **平均提示詞處理速度 (Avg Prompt Prefill)** | **27.06 tokens/s** | ARM NEON / DotProd 加速 Prefill |
| **實體記憶體佔用 (Peak RAM RSS)** | **0.62 GiB** | 含 Context KV Cache 與權重 |
| **SoC 核心峰值溫度 (Peak Temperature)** | **57.6 °C** | Active Cooler 散熱溫控狀態 |
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Standard Interaction (P512 / G128)** | 512 tokens | 128 tokens | **27.23 ± 0.03** t/s | **5.65 ± 0.01** t/s | 52.7 °C |
| **Long Context & Code Generation (P1024 / G256)** | 1024 tokens | 256 tokens | **26.89 ± 0.01** t/s | **5.77 ± 0.01** t/s | 57.6 °C |
...
2. **記憶體頻寬利用率 (Memory Bandwidth Efficiency):**
   - 模型權重為 1.88 GB，實測 Token 生成速度為 **5.71 tokens/s**。
   - 實測有效讀取頻寬為：
     $$\text{Effective Bandwidth} = 1.88 \text{ GB} \times 5.71 \text{ t/s} \approx 10.73 \text{ GB/s}$$
   - 接近 Raspberry Pi 5 LPDDR4X 記憶體匯流排在 CPU 端點的實體天花板。

3. **溫控與能耗表現 (Thermal & Energy):**
   - 壓測全程溫度維持在 **57.6 °C**，Active Cooler 運作穩定，未觸發任何降頻標誌 (`throttled=0x0`)。
