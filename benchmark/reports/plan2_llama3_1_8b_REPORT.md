# 📊 Plan 2: Native llama.cpp 基準測試報告 — `llama3.1:8b`
...
> **測試時間 (Timestamp):** `2026-09-08T12:11:04.522946`
...
| :--- | :--- | :--- |
| **模型參數規模 (Model Parameters)** | **8.03B** | 官方發行架構規模 |
| **模型量化等級 (Quantization)** | **Q4_K_M** | 4-bit K-Quants Medium |
| **模型檔案大小 (Weight Size)** | **4.58 GB** | POSIX `mmap` 零副本載入 |
| **平均解碼生成速度 (Avg Generation TPS)** | **2.46 tokens/s** | 4 核心向量並行解碼速度 |
| **平均提示詞處理速度 (Avg Prompt Prefill)** | **10.54 tokens/s** | ARM NEON / DotProd 加速 Prefill |
| **實體記憶體佔用 (Peak RAM RSS)** | **0.43 GiB** | 含 Context KV Cache 與權重 |
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Standard Interaction (P512 / G128)** | 512 tokens | 128 tokens | **10.4 ± 0.02** t/s | **2.34 ± 0.0** t/s | 57.6 °C |
| **Long Context & Code Generation (P1024 / G256)** | 1024 tokens | 256 tokens | **10.69 ± 0.04** t/s | **2.59 ± 0.0** t/s | 56.5 °C |
...
2. **記憶體頻寬利用率 (Memory Bandwidth Efficiency):**
   - 模型權重為 4.58 GB，實測 Token 生成速度為 **2.46 tokens/s**。
   - 實測有效讀取頻寬為：
     $$\text{Effective Bandwidth} = 4.58 \text{ GB} \times 2.46 \text{ t/s} \approx 11.27 \text{ GB/s}$$
