# 📊 Plan 2: Native llama.cpp 基準測試報告 — `deepseek-r1:1.5b`
...
> **測試時間 (Timestamp):** `2026-09-08T10:56:48.281863`
...
| :--- | :--- | :--- |
| **模型參數規模 (Model Parameters)** | **1.78B** | 官方發行架構規模 |
| **模型量化等級 (Quantization)** | **Q4_K_M** | 4-bit K-Quants Medium |
| **模型檔案大小 (Weight Size)** | **1.04 GB** | POSIX `mmap` 零副本載入 |
| **平均解碼生成速度 (Avg Generation TPS)** | **12.02 tokens/s** | 4 核心向量並行解碼速度 |
| **平均提示詞處理速度 (Avg Prompt Prefill)** | **59.38 tokens/s** | ARM NEON / DotProd 加速 Prefill |
| **實體記憶體佔用 (Peak RAM RSS)** | **0.59 GiB** | 含 Context KV Cache 與權重 |
| **SoC 核心峰值溫度 (Peak Temperature)** | **57.1 °C** | Active Cooler 散熱溫控狀態 |
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Standard Interaction (P512 / G128)** | 512 tokens | 128 tokens | **60.66 ± 0.13** t/s | **12.02 ± 0.07** t/s | 57.1 °C |
| **Long Context & Code Generation (P1024 / G256)** | 1024 tokens | 256 tokens | **58.1 ± 0.04** t/s | **12.03 ± 0.03** t/s | 57.1 °C |
...
2. **記憶體頻寬利用率 (Memory Bandwidth Efficiency):**
   - 模型權重為 1.04 GB，實測 Token 生成速度為 **12.02 tokens/s**。
   - 實測有效讀取頻寬為：
     $$\text{Effective Bandwidth} = 1.04 \text{ GB} \times 12.02 \text{ t/s} \approx 12.5 \text{ GB/s}$$
   - 接近 Raspberry Pi 5 LPDDR4X 記憶體匯流排在 CPU 端點的實體天花板。

3. **溫控與能耗表現 (Thermal & Energy):**
   - 壓測全程溫度維持在 **57.1 °C**，Active Cooler 運作穩定，未觸發任何降頻標誌 (`throttled=0x0`)。
