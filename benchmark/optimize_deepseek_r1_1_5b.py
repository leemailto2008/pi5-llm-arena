# f:\12_prj_raspi5\benchmark\optimize_deepseek_r1_1_5b.py
"""
Benchmark & Optimization Pipeline for DeepSeek-R1-1.5B on Raspberry Pi 5
1. Verifies CPU Governor is locked at 'performance' (2.40 GHz).
2. Builds optimized model variant 'deepseek-r1:1.5b-opt' with customized Modelfile (num_thread=4, num_ctx=4096, R1-tuned reasoning params).
3. Executes real comparative benchmarks between baseline deepseek-r1:1.5b and deepseek-r1:1.5b-opt on discrete math and dynamic programming tasks.
...
PROMPTS = [
    {
        "category": "Discrete Mathematics & Pigeonhole Proof",
        "prompt": "Prove step-by-step using the Pigeonhole Principle: In any group of 6 people, there are either 3 mutual friends or 3 mutual strangers. Show the reasoning chain clearly."
    },
    {
        "category": "Dynamic Programming & Memory Efficiency",
        "prompt": "Solve the 0/1 Knapsack Problem with weights [2, 3, 4, 5] and values [3, 4, 5, 6] with a maximum capacity of 5. Show the DP table transition and state compression optimization."
    }
]
...
def create_optimized_modelfile(ssh: paramiko.SSHClient):
    print("[*] Creating optimized Modelfile for deepseek-r1:1.5b via Ollama CLI...")
    modelfile_content = (
        ...
        "SYSTEM \"\"\"You are an ultra-fast edge reasoning engine. Provide concise, step-by-step mathematical reasoning and explicit proofs.\"\"\"\n"
    )
    sftp = ssh.open_sftp()
    with sftp.open("/home/pi/Modelfile.r1_15b", "w") as f:
        f.write(modelfile_content)
    sftp.close()
    
    stdin, stdout, stderr = ssh.exec_command("ollama create deepseek-r1:1.5b-opt -f /home/pi/Modelfile.r1_15b")
...
def main():
    print("=================================================================")
    print(" Pi 5 DeepSeek-R1 1.5B Optimization & Verification Benchmark")
    print("=================================================================")
    ...
    try:
        ...
        for prompt_data in PROMPTS:
            res_base = run_benchmark("deepseek-r1:1.5b", prompt_data, ssh)
            results.append(res_base)
            
            time.sleep(3)
            
            res_opt = run_benchmark("deepseek-r1:1.5b-opt", prompt_data, ssh)
            results.append(res_opt)
            
            time.sleep(3)
            
        output_file = os.path.join(os.path.dirname(__file__), "deepseek_r1_1_5b_opt_benchmark_results.json")
